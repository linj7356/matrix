...
public class MainFragment extends Fragment implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    private void animateDialog(View dialogView, boolean open, final Dialog dialog) {
        ...
        int endRadius = (int) Math.hypot(w, h);//对角线：diagonal

        int cx = (int) (fab_report.getX() + (fab_report.getWidth()/2)); //animation 从哪开始（x轴）
        int cy = (int) (fab_report.getY())+ fab_report.getHeight() + 56;

        if(open){
            Animator revealAnimator = ViewAnimationUtils.createCircularReveal(view, cx,cy, 0, endRadius);//从0往外扩散
            view.setVisibility(View.VISIBLE);
            revealAnimator.setDuration(500);//0.5s
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CAPTURE_IMAGE &&
            ...
            if (data != null && data.getExtras() != null) {
                //BitMap: Android's images' storage format
